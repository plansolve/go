...
func TestStopSendsDeleteAndStampsJobID(t *testing.T) {
	...
	if gotPath != "/api/v1/professionalservices/job-stop" {
		t.Errorf("expected path '/api/v1/professionalservices/job-stop', got '%s'", gotPath)
	}
	if gotKey != "test-key" {
...
func TestAnalyzeCallsAnalyzeEndpoint(t *testing.T) {
	...
	if gotPath != "/api/v1/professionalservices/job-an/analyze" {
		t.Errorf("expected path '/api/v1/professionalservices/job-an/analyze', got '%s'", gotPath)
	}
